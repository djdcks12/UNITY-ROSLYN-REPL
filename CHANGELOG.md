...
- `SingletonScanner` now detects **any** self-returning public static member regardless of name (`Instance`, `it`, `I`, `Self`, `Current`, `Singleton`, etc.), so plain C# managers like `LobbyFriendManager.it` are surfaced. Compiler-generated closure / lambda-cache types (names starting with `<`) are filtered out so they don't flood the list.
