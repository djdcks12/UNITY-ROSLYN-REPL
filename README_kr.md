...
`Packages/manifest.json`에 패키지를 추가합니다.
...
패키지에는 OpenUPM 메타데이터가 포함되어 있습니다. OpenUPM에 게시된 뒤에는 scoped registry로 설치할 수 있습니다.
...
게시 전에는 Git URL 또는 로컬 디스크 설치 방식을 사용하세요.
...
저장은 Unity `EditorPrefs`를 사용하되 프로젝트 경로 기반 discriminator를 붙입니다. 데이터는 현재 PC에만 있고 소스 컨트롤에 커밋되지 않습니다.
